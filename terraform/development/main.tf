...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/t-and-l-activity-history-frontend/state"
  }
}
resource "aws_s3_bucket" "frontend-bucket-development" {
  bucket = "lbh-housing-tl-activity-history-frontend-development.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
...
module "cloudfront-development" {
  ...
  origin_id = "mtfh-t-and-l-activity-history-frontend"
