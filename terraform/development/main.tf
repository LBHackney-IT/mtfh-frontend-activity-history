...
terraform {
  backend "s3" {
    ...
    encrypt = true
    region  = "eu-west-2"
    key     = "services/t-and-l-activity-frontend/state"
  }
}
resource "aws_s3_bucket" "frontend-bucket-development" {
  bucket = "lbh-housing-tl-activity-frontend-development.hackney.gov.uk"
  acl    = "private"
  versioning {
    enabled = true
  }
...
module "cloudfront-development" {
  ...
  origin_id = "mtfh-t-and-l-activity-frontend"
  s3_bucket_arn = aws_s3_bucket.frontend-bucket-development.arn
  s3_bucket_id = aws_s3_bucket.frontend-bucket-development.id
  orginin_access_identity_desc = "T&L activity history frontend cloudfront identity"
  cname_aliases = []
...
resource "aws_ssm_parameter" "cdn" {
  name  = "/housing-tl/development/activities-app-url"
  type  = "String"
