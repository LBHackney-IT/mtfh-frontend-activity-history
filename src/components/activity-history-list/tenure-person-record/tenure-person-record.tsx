...
import { locale, HouseholdMember } from '@services';
import {
    Activity,
...
const getHouseholdMemberChanged = (
    ...
    const householdMemberIdChanged = oldDataMemberIds
        ...
        .concat(
            newDataMemberIds.filter(
                newMemberId => !oldDataMemberIds.includes(newMemberId)
            )
        )[0];
    return newDataMemberIds.includes(householdMemberIdChanged)
        ? newData.householdMembers.find(
              member => member.id === householdMemberIdChanged
          )
        : oldData.householdMembers.find(
              member => member.id === householdMemberIdChanged
          );
};
...
const PersonAddedOrRemoved = ({
    ...
    <div>
        {title}
        {Object.entries(tenurePerson).map(
            ([tenurePersonKey, tenurePersonVal], index) => (
                <p key={index}>
                    {tenurePersonVal.field}{' '}
                    <b>
                        {tenurePersonVal.output(
                            householdMemberChanged[tenurePersonKey]
                        )}
                    </b>
                </p>
            )
